...
**Status (2026-07-22): M1 local vertical slice; not deployed, not live, not audited, not mainnet-ready. Live app URL: none.** The dashboard refuses to invent packet activity. Solidity compiles against pinned official LayerZero packages; the adapter and an EndpointV2-compatible OApp lifecycle are exercised on a local EVM. A supervised composition root now assembles durable ingestion, bounded poison-packet quarantine, operator-safe requeue, independent verification, strict request construction, GenLayer finality polling and the same-origin dashboard; restart restores the full GenLayer request binding, not only its transaction ID. GenLayer direct-mode, approved account-provider construction, isolated signer transport and real ULN302 integration remain required before a standalone daemon or deployment.
...
Alert on stuck stage age, repeated ingestion failure, any dead-letter entry, RPC disagreement, reorg, evidence staleness, GenLayer undetermined/appeal state, signer divergence, quorum latency, adapter submission failure and destination execution failure. A packet is acknowledged only after successful ingestion or durable quarantine. Investigate before calling the local `RecoveryService.requeue(txHash)` operation; it restores the packet before clearing the incident record. The browser exposes read-only sanitized quarantine metadata and cannot requeue. Pause intake/signers on integrity alerts. Remove the optional DVN via OApp governance if availability is affected. Rotate compromised signers using threshold administration. Do not overwrite decisions or pretend executed actions are reversible.

## Limitations

Gasolina does not document asynchronous pending/retry behavior for extra context. Test confirmation and ingestion-attempt values are placeholders. DVN onboarding/VID and receive-library/DVN addresses require confirmation; the 2026-07-22 web re-audit was blocked by the local network permission boundary, so `config/networks.json` remains dated 2026-07-21. The pinned GenLayer SDK adapter is fixture-tested but has not contacted Studio or a testnet. The runtime is programmatically composable but is not a standalone daemon until an approved GenLayer account provider is integrated. Signer objects are abstractions, not isolated infrastructure, and the composed runtime intentionally creates zero signers. SQLite listener/job/recovery state is single-node rather than HA consensus storage; acknowledged delivery is at-least-once and relies on GUID idempotency rather than distributed exactly-once semantics. Recovery is a programmatic local service, not a hardened authenticated CLI, and retry is tick-based without exponential delay. Evidence fetching enforces an exact hostname allowlist but production still requires DNS/IP controls and restricted egress to address DNS rebinding. The local Endpoint harness is behavioral test code, not LayerZero protocol code. The dashboard is deliberately read-only: it inspects coordinator lifecycle and sanitized quarantine state and does not initiate OApp wallet transactions, recovery, or signing actions.
