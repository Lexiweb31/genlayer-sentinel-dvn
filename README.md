...
The only signer wire version is `sentinel-signer/v2`. It carries a bounded GenLayer transaction/evidence/action/policy witness, and every isolated signer must independently require `FINALIZED`/`7`, a successful exact `evaluate` call to the pinned policy contract, and the strict request-bound record before its key is invoked. Replay state binds both the adapter execution digest and authorization digest. This repository still provides injected transport and transaction-reader interfaces rather than a listening mTLS daemon, official live GenLayer reader, HSM integration or five independent operators.
