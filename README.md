...
The route split changes presentation only. In hosted mode the console has no coordinator binding and cannot deploy contracts, onboard a DVN, obtain GenLayer finality, operate a signer quorum, or submit a governed action. Only the explicit loopback `demo:local` capability can enable the local wallet action workspace; visiting `/console/` does not grant that capability. A separate approved deployment is required before claiming that the prior hosted URL serves these routes.
