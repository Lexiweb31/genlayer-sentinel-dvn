...
The hosted surface is the same static, self-origin dashboard package verified by the repository tests. Its local pathway-artifact inspector works entirely in browser memory. It intentionally has no hosted coordinator API binding, so packet, runtime, demo-wallet, recovery, and delivery panels fail closed as unavailable rather than showing simulated data.
...
- `apps/dashboard/`: self-contained same-origin operations app with a Sentinel hero, in-memory local pathway-artifact inspector, capability-gated local wallet action workspace, and read-only coordinator workspace; no remote asset dependency or simulated job fallback.
