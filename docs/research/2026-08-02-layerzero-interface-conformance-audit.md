...
`SentinelDVNAdapter` now explicitly inherits that pinned interface. Its `assignJob` is payable solely to preserve the required ABI and immediately rejects `msg.value != 0`; `getFee` returns zero for its single configured destination. Contract tests compare the generated selectors and mutability against an independently declared ABI and prove authorized zero-fee assignment, exact refusal errors and precedence, nonzero-value rejection, and zero retained balance through the job hook. Because native value can still be force-sent outside the hook, a separate permissionless `recoverNative` operation returns the complete balance only to the immutable authorized message library; callers cannot redirect it. Slither is invoked with ignored findings shown so inline directives cannot conceal a production High or Medium result.
