...
const html=fs.readFileSync("apps/dashboard/index.html","utf8"),js=fs.readFileSync("apps/dashboard/src/app.js","utf8"),timeline=fs.readFileSync("apps/dashboard/src/timeline.js","utf8"),demo=fs.readFileSync("apps/dashboard/src/demo-entry.ts","utf8"),bundle=fs.readFileSync("dist/apps/dashboard/demo.js","utf8");
...
for(const token of ["fetch(\"/api/demo/config\"","sentinel:guid-observed","WalletActionClient","COORDINATOR_STAGE","/api/jobs/","/api/deliveries"])if(!demo.includes(token))throw new Error(`missing coordinator-bound demo behavior: ${token}`);
