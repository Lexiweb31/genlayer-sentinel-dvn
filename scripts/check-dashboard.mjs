...
for(const token of ["fetch(\"/api/demo/config\"","sentinel:guid-observed","sentinel:demo-bootstrap","WalletActionClient","COORDINATOR_STAGE","executionFailureCode","/api/jobs/","/api/deliveries"])if(!demo.includes(token))throw new Error(`missing coordinator-bound demo behavior: ${token}`);
