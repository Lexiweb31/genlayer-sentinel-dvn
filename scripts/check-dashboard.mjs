...
const html=fs.readFileSync("apps/dashboard/index.html","utf8"),js=fs.readFileSync("apps/dashboard/src/app.js","utf8"),timeline=fs.readFileSync("apps/dashboard/src/timeline.js","utf8"),runtime=fs.readFileSync("apps/dashboard/src/runtime-status.js","utf8"),demo=fs.readFileSync("apps/dashboard/src/demo-entry.ts","utf8"),wallet=fs.readFileSync("apps/dashboard/src/wallet-action.ts","utf8"),bundle=fs.readFileSync("dist/apps/dashboard/demo.js","utf8");
for(const id of ["runtime-mode","runtime-status-badge","runtime-lifecycle","runtime-lease","runtime-phase","runtime-heartbeat","runtime-last-tick","runtime-recovery-posture","live-notice","connection-status","timeline","job-select","inspector","packet-details","verification-details","policy-details","signer-details","quarantine-status","dead-letters","delivery-status","deliveries","recovery-action-status","recovery-actions","demo-workspace","demo-status","demo-connect","demo-record-label","demo-argument","demo-quote","demo-fee","demo-send","demo-transaction","demo-guid","demo-message"])if(!html.includes(`id="${id}"`))throw new Error(`missing dashboard target ${id}`);
for(const token of ["fetch(\"/health\"","fetch(\"/api/runtime-status\"","fetch(\"/api/jobs\"","fetch(\"/api/dead-letters\"","fetch(\"/api/deliveries\"","fetch(\"/api/recovery-actions\"","LOCAL TEST","Signing intent","Destination submission","OPERATOR RECOVERY","3 of 5 approvals","deliveryByGuid","SIGNING","SUBMITTED","COORDINATOR UNAVAILABLE","No simulated state is shown","job.verifications","job.result","job.signers"])if(!`${html}\n${js}`.includes(token))throw new Error(`missing honest live-data behavior: ${token}`);
for(const token of ["Live observation from this coordinator process only.","not failover","high availability","recovery authorization"])if(!html.includes(token))throw new Error(`missing runtime observation limitation: ${token}`);
for(const token of ["NOT APPLICABLE · LOCAL FIXTURE","REQUIRES_OFFLINE_VERIFICATION","invalid runtime status"])if(!runtime.includes(token))throw new Error(`missing closed runtime presentation: ${token}`);
...
const runtimeSection=html.match(/<section class="runtime-observation"[\s\S]*?<\/section>/i)?.[0];if(!runtimeSection)throw new Error("missing live runtime observation section");
if(/<(button|form|input|select)\b/i.test(runtimeSection))throw new Error("runtime observation dashboard must remain read-only");
