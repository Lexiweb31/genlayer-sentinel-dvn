...
for(const token of ["fetch(\"/health\"","fetch(\"/api/jobs\"","fetch(\"/api/dead-letters\"","fetch(\"/api/deliveries\"","LOCAL TEST","SIGNING","COORDINATOR UNAVAILABLE","No simulated state is shown","job.verifications","job.result","job.signers"])if(!js.includes(token))throw new Error(`missing honest live-data behavior: ${token}`);
