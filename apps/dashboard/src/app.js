...
let jobs=[],selectedGuid=typeof initialConsoleGuid==="string"&&initialConsoleGuid?initialConsoleGuid:undefined,observedGuid,deliveryByGuid=new Map(),pollingStarted=false,consoleQuery=initialConsoleQuery;
...
function selectJob(job,{updateUrl=false}={}){selectedGuid=job.packet.guid;select.value=selectedGuid;if(updateUrl)replaceSelectedGuid(selectedGuid);render(job);renderMessageList(jobs.filter(item=>matchesConsoleQuery(item,consoleQuery)))}
...
function renderMessageList(visibleJobs){messageList.replaceChildren(...visibleJobs.map(job=>{const row=document.createElement("tr");row.className="message-row";row.setAttribute("aria-current",sameGuid(job.packet.guid,selectedGuid)?"true":"false");for(const [value,kind] of [[job.packet.srcEid,"origin"],[job.packet.dstEid,"destination"],[job.packet.guid,"identifier"],[messageObservedAt(job),"observed"],[job.stage,"stage"]]){const cell=document.createElement("td");if(kind==="identifier"){const button=document.createElement("button");button.type="button";button.textContent=short(value);button.title=String(value);button.setAttribute("aria-label",`Inspect packet ${value}`);button.addEventListener("click",()=>selectJob(job,{updateUrl:true}));cell.append(button)}else{cell.textContent=String(value??"—");if(kind==="stage")cell.className="message-stage"}row.append(cell)}return row}))}
...
function populate(){const visibleJobs=jobs.filter(job=>matchesConsoleQuery(job,consoleQuery));renderMessageList(visibleJobs);select.replaceChildren(...visibleJobs.map(job=>{const option=document.createElement("option");option.value=job.packet.guid;option.textContent=`${short(job.packet.guid)} · ${job.stage}`;return option}));if(!visibleJobs.length){showEmpty(consoleQuery?"No observed packet matches this query.":"No packets are currently observed.",consoleQuery?"NO QUERY MATCH":"NO PACKETS DETECTED");return}consoleEmpty.hidden=true;select.disabled=false;const observed=visibleJobs.find(job=>sameGuid(job.packet.guid,observedGuid)),prior=visibleJobs.find(job=>sameGuid(job.packet.guid,selectedGuid)),job=observed??prior??visibleJobs.at(-1);selectJob(job)}
...
async function refresh(){try{const response=await fetch("/api/jobs",{headers:{accept:"application/json"},cache:"no-store"});if(!response.ok)throw new Error(`status ${response.status}`);jobs=await response.json();if(!Array.isArray(jobs))throw new Error("invalid response");refreshed.textContent=`Refreshed ${new Date().toLocaleTimeString()}`;if(jobs.length===0){showEmpty("No packets are currently observed.");notice.textContent="Coordinator connected. No packet jobs exist.";return}populate()}catch(error){unavailable(`No coordinator data: ${error.message}. No simulated state is shown.`)}}
...
select.addEventListener("change",()=>{observedGuid=undefined;const job=jobs.find(item=>sameGuid(item.packet.guid,select.value));if(job)selectJob(job,{updateUrl:true})});
window.addEventListener("sentinel:guid-observed",event=>{const guid=event.detail?.guid;if(typeof guid!=="string"||!/^0x[0-9a-fA-F]{64}$/.test(guid))return;observedGuid=guid.toLowerCase();const job=jobs.find(item=>item.packet.guid.toLowerCase()===observedGuid);if(job){selectedGuid=job.packet.guid;select.value=selectedGuid;render(job)}});
