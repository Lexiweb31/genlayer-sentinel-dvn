...
import {createHeroMotionController} from "./hero-motion.js";
...
const heroMotionController=heroVideo&&typeof window.matchMedia==="function"?createHeroMotionController({video:heroVideo,mediaQuery:window.matchMedia("(prefers-reduced-motion: reduce)")}):{dispose(){}};
window.addEventListener("pagehide",()=>{pathwayController.dispose();heroMotionController.dispose()},{once:true});
let jobs=[],selectedGuid,observedGuid,deliveryByGuid=new Map(),pollingStarted=false;
...
function populate(){const prior=selectedGuid,observed=jobs.find(job=>job.packet.guid.toLowerCase()===observedGuid);select.replaceChildren(...jobs.map(job=>{const option=document.createElement("option");option.value=job.packet.guid;option.textContent=`${short(job.packet.guid)} · ${job.stage}`;return option}));select.disabled=false;selectedGuid=observed?.packet.guid??(jobs.some(job=>job.packet.guid===prior)?prior:jobs.at(-1).packet.guid);select.value=selectedGuid;render(jobs.find(job=>job.packet.guid===selectedGuid))}
...
select.addEventListener("change",()=>{observedGuid=undefined;selectedGuid=select.value;const job=jobs.find(item=>item.packet.guid===selectedGuid);if(job)render(job)});
