...
import type {Hex, Packet} from "../../../packages/core/src/types.js";
...
export interface PacketReceiptVerification {provider:string;blockHash:Hex;payloadHash:Hex;confirmations:bigint}
export interface PacketReceiptVerifier {verify(packet:Packet):Promise<PacketReceiptVerification[]>}
...
export class IndependentRpcPacketVerifier implements PacketReceiptVerifier {
  constructor(private urls:string[],private endpoint:string,private requiredConfirmations:bigint,private rpc:RpcFetch=safeJsonRpc) {
  ...
  async verify(packet:Packet):Promise<PacketReceiptVerification[]> {
