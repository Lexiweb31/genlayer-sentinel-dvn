...
export class RecoveryService implements DeadLetterReader{
 constructor(private pathwayKey:string,private store:RecoveryStore,private inbox:PacketInbox){}
 listDead():Promise<DeadLetter[]>{return this.store.listDead(this.pathwayKey)}
 async requeue(transactionHash:string):Promise<boolean>{const record=await this.store.findDead(this.pathwayKey,transactionHash);if(!record)return false;await this.inbox.requeue(record.packet);await this.store.resolve(this.pathwayKey,record.packet.transactionHash);return true}
}
