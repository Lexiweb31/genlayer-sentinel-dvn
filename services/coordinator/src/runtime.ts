export interface RuntimeDependencies {restore():Promise<void>;ingest():Promise<void>;pollFinality():Promise<void>;planDeliveries():Promise<void>;deliver():Promise<void>;listen():Promise<void>;closeServer():Promise<void>;closeStores():void;report(error:unknown):void;intervalMs:number;schedule?(task:()=>Promise<void>,intervalMs:number):()=>void;}
...
export class SentinelRuntime {
  ...
  private runTick():Promise<void>{if(this.tick||!this.started||this.stopping)return this.tick??Promise.resolve();const work=(async()=>{try{await this.dependencies.ingest();await this.dependencies.pollFinality();await this.dependencies.planDeliveries();await this.dependencies.deliver()}catch(error){this.dependencies.report(error)}})();this.tick=work.finally(()=>{this.tick=undefined});return this.tick}
