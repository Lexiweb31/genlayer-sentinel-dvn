...
import {collectQuorum,type SignerService,type SignatureShare,type SigningEnvelope} from "./signing.js";
...
export class Coordinator {
  ...
  constructor(private verifier: PacketVerifier, private genlayer: GenLayerFinality, readonly signers: SignerService[], private quorum = 3, private minimumConfirmations = 15n,private store?:JobStore) {}
  ...
  async recordQuorum(guid:string,addresses:Hex[]):Promise<void>{
    ...
    if(job.snapshot.stage==="QUORUM_REACHED"){if(JSON.stringify(job.snapshot.signers)!==JSON.stringify(normalized))throw new Error("durable quorum mismatch");return}
