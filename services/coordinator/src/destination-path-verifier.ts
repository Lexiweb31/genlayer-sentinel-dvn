...
export interface DestinationPathObservation{
  ...
  rawAppUln:UlnObservation;resolvedUln:UlnObservation;sourcePeer:Hex;adapter:AdapterObservation;
}
...
export async function readDestinationPathObservation(input:DestinationPathObservationInput,reader:PinnedStateReader):Promise<DestinationPathObservation>{
  ...
  const receive=result(endpointInterface,"getReceiveLibrary",await call(reader,checked.endpoint,data(endpointInterface,"getReceiveLibrary",[checked.oapp,checked.srcEid])));
  const receiveLibrary=address(receive[0]),isDefaultReceiveLibrary=boolean(receive[1]);await code(reader,receiveLibrary);
  const supportedEid=boolean(result(receiveInterface,"isSupportedEid",await call(reader,receiveLibrary,data(receiveInterface,"isSupportedEid",[checked.srcEid])))[0]);
  const resolvedUln=ulnObservation(result(receiveInterface,"getUlnConfig",await call(reader,receiveLibrary,data(receiveInterface,"getUlnConfig",[checked.oapp,checked.srcEid])))[0]);
  const rawAppUln=ulnObservation(result(receiveInterface,"getAppUlnConfig",await call(reader,receiveLibrary,data(receiveInterface,"getAppUlnConfig",[checked.oapp,checked.srcEid])))[0]);
  const sourcePeer=bytes32(result(oappInterface,"peers",await call(reader,checked.oapp,data(oappInterface,"peers",[checked.srcEid])))[0]);
  return{endpoint:checked.endpoint,oapp:checked.oapp,srcEid:checked.srcEid,receiveLibrary,isDefaultReceiveLibrary,supportedEid,rawAppUln,resolvedUln,sourcePeer,adapter:await readAdapterObservation(checked.adapter,checked.authorizedSigners,reader)};
}
...
export class IndependentDestinationPathVerifier implements DestinationPathVerifier {
  ...
  private async observe():Promise<VerifiedDestinationPath>{
    ...
    const observations=await Promise.all(heads.map(head=>this.observation(head.url,observedBlockHash)));if(observations.some(value=>stable(value)!==stable(observations[0])))throw new Error("destination provider disagreement");const value=observations[0]!;
    this.assertPinned(heads[0]!.chainId,value);const requiredHash=keccak256(coder.encode(["address[]"],[value.rawAppUln.requiredDvns])),optionalHash=keccak256(coder.encode(["address[]"],[value.rawAppUln.optionalDvns])),authorizedSigners=this.config.authorizedSigners.map(normalizeAddress),signerHash=keccak256(coder.encode(["address[]"],[authorizedSigners]));
    const configurationDigest=keccak256(coder.encode(["uint256","bytes32","uint256","uint32","address","address","address","address","uint64","bytes32","bytes32","uint8","bytes32","uint256"],[observedBlockNumber,observedBlockHash,heads[0]!.chainId,this.config.srcEid,this.config.endpoint,this.config.receiveLibrary,this.config.oapp,this.config.adapter,value.rawAppUln.confirmations,requiredHash,optionalHash,value.rawAppUln.optionalDvnThreshold,signerHash,value.adapter.quorum])) as Hex;
    return{observedBlockNumber,observedBlockHash,chainId:heads[0]!.chainId,srcEid:this.config.srcEid,endpoint:normalizeAddress(this.config.endpoint),receiveLibrary:value.receiveLibrary,oapp:normalizeAddress(this.config.oapp),adapter:normalizeAddress(this.config.adapter),confirmations:value.rawAppUln.confirmations,requiredDvns:value.rawAppUln.requiredDvns,optionalDvns:value.rawAppUln.optionalDvns,optionalDvnThreshold:value.rawAppUln.optionalDvnThreshold,authorizedSigners,quorum:3,configurationDigest};
  }
  ...
  private async observation(url:string,blockHash:Hex):Promise<DestinationPathObservation>{
