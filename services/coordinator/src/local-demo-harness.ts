...
export async function startLocalDemo(rawOptions:LocalDemoOptions):Promise<LocalDemoSession>{
  ...
  let stopped:Promise<void>|undefined,timer:NodeJS.Timeout|undefined,activeTick:Promise<void>|undefined,shuttingDown=false;
  try{
    ...
    const finality=new LocalDemoFinality(authority),metrics={signerCalls:0,destinationSubmissions:0};
    ...
    const jobStore=new SqliteJobStore(databasePath);register(()=>jobStore.close());
    const listenerStore=new SqliteListenerStore(databasePath);register(()=>listenerStore.close());
    const recoveryStore=new SqliteRecoveryStore(databasePath);register(()=>recoveryStore.close());
    const outbox=new SqliteVerificationOutbox(databasePath,authorizedSigners,3);
    register(()=>outbox.close());
    coordinator=new Coordinator(new LocalEdrPacketVerifier(rpc,(await sourceEndpoint.getAddress()) as Hex,1n),finality,signerServices,3,1n,jobStore);
    const listener=new PacketFeeListener(
      new JsonRpcLogSource(evm.rpcUrl),(await sourceEndpoint.getAddress()) as Hex,(await sourceEndpoint.getAddress()) as Hex,
    ...
    const factory=new PolicyRequestFactory({
      ...
      sendLibrary:(await sourceEndpoint.getAddress()) as Hex,sentinelDvn:adapterAddress,
    ...
    const pathVerifier=new LocalEdrPathVerifier(rpc,{
      chainId:31337n,srcEid:sourceEid,endpoint:(await destinationEndpoint.getAddress()) as Hex,
      receiveLibrary:verificationTargetAddress,oapp:destinationAddress,adapter:adapterAddress,sourcePeer,
      confirmations:1n,requiredDvns:[(await evm.signers[10]!.getAddress()).toLowerCase() as Hex],
      optionalDvns:[adapterAddress],optionalDvnThreshold:1,authorizedSigners,quorum:3
    });
    ...
    const execution=new LocalOAppExecutionConfirmer(coordinator,rpc,{
      from:(await evm.signers[11]!.getAddress()).toLowerCase() as Hex,
      endpoint:(await destinationEndpoint.getAddress()).toLowerCase() as Hex,
      oapp:destinationAddress,actionTarget:targetAddress
    });
    ...
    const capability=parseDemoCapability({
      ...
      sourceOApp:sourceAddress,sourceEndpoint:(await sourceEndpoint.getAddress()).toLowerCase(),
    ...
    register(()=>closeServer(server));
    await listen(server,options.appPort,options.appHost);
    const addressInfo=server.address();
    if(!addressInfo||typeof addressInfo==="string")throw new Error("local dashboard address unavailable");
    const appUrl=`http://${options.appHost}:${addressInfo.port}`;

    const runTick=async()=>{
      await rpc("evm_mine",[]);
      await ingestion.pollOnce();
      await coordinator.pollPending();
      await planner.pollOnce();
      await destinationWorker.pollOnce();
    };
    ...
    return{appUrl,rpcUrl:evm.rpcUrl,capability,provider:evm.provider,coordinator,outbox,sourceOApp,destinationOApp,actionTarget,metrics,tickOnce,stop};
